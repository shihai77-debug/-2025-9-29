...
This mode tells the image generator:
...
沈泊川：年轻清洁服务公司老板/现场负责人，灰绿或深灰工作衬衫外套、深灰T恤、棕色工装裤、棕色工作靴、卡其色实用背包/工具包；摩托车和头盔只作为交通道具，不是黑皮骑手造型。
```
...
```text
人物采用浅灰低对比导演参考图风格，身体和服装轮廓清楚，脸部保留外轮廓和头发方向，但不要画清楚眼睛、鼻子、嘴巴。背景空间可以更清楚，人物不要过度写实，不要照片级面部细节。
```
...
For important dialogue, still create a clean keyframe or face lock image from the turnaround sheet.

## Prompt Snippet

Use this snippet in storyboard/reference image prompts when face accuracy is failing:
